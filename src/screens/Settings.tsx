...
type Tab = 'users' | 'roles' | 'email';

export function ScreenSettings() {
  const { user: currentUser, isAdmin } = usePermissions();
  const department = currentUser.department;
  const departmentScreens = DEPARTMENT_SCREENS[department];
  // The LTA inspection email designer lives in the Charger Registry department.
  const showEmailDesigner = department === 'pm' && isAdmin;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 4, background: C.white, borderRadius: 12, padding: 4, border: '1px solid #EBEBEB', alignSelf: 'flex-start' }}>
        {([
          ...
          ...(showEmailDesigner ? [['email', 'Email Designer'] as [Tab, string]] : []),
...
// ── LTA inspection email designer (Charger Registry admins) ───────
// Edits the subject + HTML body sent with Form A / Form D inspection + invoice.
// Placeholders: {{charger}} {{form_type}} {{site}} {{customer}} {{date}}.

interface LtaTemplate { form_type: 'A' | 'D'; subject: string; body: string; }

function LtaEmailDesigner() {
  ...
  if (loading) return <div style={{ color: C.slate, fontSize: 13 }}>Loading templates…</div>;
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
      <div style={{ background: C.white, borderRadius: 16, border: '1px solid #EBEBEB', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <input value={current.subject} onChange={(e) => patch({ subject: e.target.value })} style={field} />
        </div>
        <div>
          <label style={label}>Body (HTML)</label>
          <textarea value={current.body} onChange={(e) => patch({ body: e.target.value })} rows={12}
